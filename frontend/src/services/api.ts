  ...
  async createBooking(bookingData: Partial<Booking>): Promise<Booking> {
    ...
    try {
      ...
      if (errorData.start_time) {
        throw new Error(errorData.start_time[0] || errorData.start_time);
      }
      if (errorData.start_date) {
        throw new Error(errorData.start_date[0] || errorData.start_date);
      }
      if (errorData.non_field_errors) {
        throw new Error(errorData.non_field_errors[0] || errorData.non_field_errors);
      }
      ...
      // If JSON parsing failed, it's likely an HTML error page
      throw new Error('Server error. Please try selecting a future time.');
    }
  }
