...
const BookingBoard = () => {
  ...
  // Use rooms and bookings from context instead of local state
  ...
  const updateBooking = async (bookingData) => {
    try {
      ...
      // Use API to update booking
      await apiUpdateBooking(editingBooking.id, apiBookingData);
    ...
    // Send procurement notification if there are orders (if implemented)
    if (bookingData.procurementOrders && bookingData.procurementOrders.length > 0) {
      sendProcurementNotification({ ...editingBooking, ...bookingData });
    }
  ...
  const approveBooking = async (bookingId) => {
    ...
    if (window.confirm('Are you sure you want to approve this booking?')) {
      ...
      // In a real API implementation, there would be an approve endpoint
      console.log('Approve booking functionality needs API endpoint implementation');
  ...
  const rejectBooking = async (bookingId) => {
    ...
    if (reason !== null) { // User didn't cancel the prompt
      ...
      // In a real API implementation, there would be a reject endpoint
      console.log('Reject booking functionality needs API endpoint implementation');
  ...
  const renderSlotContent = (room, time) => {
    ...
    // Debug logging for the first room and time slot
    if (room.id === rooms[0]?.id && time === '08:00') {
      console.log('Checking slot:', room.name, time, 'Booked:', isBooked, 'Booking:', booking);
      console.log('All bookings:', bookings);
    }

    if (isBooked && booking) {
      // Handle both API and old format data
      const bookingTimeLabel = booking.start_time || booking.time || booking.startTime || time;
      const bookingTitle = booking.purpose || booking.title || 'Meeting';
      const bookingOrganizer = booking.user_name || booking.organizer || 'User';
      const approvalStatus = booking.approval_status || booking.approvalStatus || 'pending';

      return (
        <div className={`modern-time-slot booked ${booking.bookingType || 'hourly'} ${approvalStatus}`}>
          ...
          <div className={`slot-status booked-status ${approvalStatus}`}>
            <span className="status-text">
              {approvalStatus === 'approved' && 'Approved'}
              {approvalStatus === 'rejected' && 'Rejected'}
              {(!approvalStatus || approvalStatus === 'pending') && 'Pending'}
            </span>
            <div className={`status-pulse ${approvalStatus}`}></div>
          </div>
  ...
  const isSlotBooked = (roomId, time) => {
    ...
    const matchingBooking = bookings.some(booking => {
      ...
      const bookingStartDate = new Date(booking.start_date || booking.startDate || booking.date);
      const bookingEndDate = new Date(booking.end_date || booking.endDate || booking.date);
      const currentDateObj = new Date(currentDate);

      console.log(`📅 Date check: booking ${bookingStartDate.toISOString().split('T')[0]} to ${bookingEndDate.toISOString().split('T')[0]}, current ${currentDateObj.toISOString().split('T')[0]}`);
      ...
      if (currentDateObj < bookingStartDate || currentDateObj > bookingEndDate) {
        console.log(`❌ Date rejected: current date outside booking range`);
        return false;
      }
      console.log(`✅ Date accepted: current date within booking range`);

      // Get booking times (handle both API and old formats)
      const bookingStartTime = booking.start_time || booking.time;
      const bookingEndTime = booking.end_time || (booking.time && booking.duration ?
        `${parseInt(booking.time.split(':')[0]) + booking.duration}:00` : null);

      console.log(`⏰ Time check: booking ${bookingStartTime}-${bookingEndTime}, checking slot ${time}`);

      // If we have specific start/end times, check time overlap
      if (bookingStartTime && bookingEndTime) {
        // Normalize time formats - ensure both have seconds (HH:MM:SS)
        const normalizeTime = (timeStr) => {
          if (!timeStr) return timeStr;
          const parts = timeStr.split(':');
          if (parts.length === 2) {
            return `${timeStr}:00`; // Add :00 seconds if missing
          }
          return timeStr;
        };

        const normalizedSlotTime = normalizeTime(time);
        const normalizedBookingStart = normalizeTime(bookingStartTime);
        const normalizedBookingEnd = normalizeTime(bookingEndTime);

        console.log(`🔍 Normalized time check: "${normalizedSlotTime}" >= "${normalizedBookingStart}" && "${normalizedSlotTime}" < "${normalizedBookingEnd}"`);

        const isOverlap = normalizedSlotTime >= normalizedBookingStart && normalizedSlotTime < normalizedBookingEnd;
        console.log(`✨ Final overlap result: ${isOverlap}`);
        return isOverlap;
      }

      // For old format hourly bookings
      if (booking.bookingType === 'hourly' && booking.time) {
        const bookingStartIndex = getTimeSlotIndex(booking.time);
        const bookingEndIndex = bookingStartIndex + (booking.duration || 1);
    ...
    console.log(`Slot booked result: ${matchingBooking}`);
  ...
  const getBookingDetails = (roomId, time) => {
    ...
    return bookings.find(booking => {
      ...
      if (currentUser && (currentUser.role === 'super_admin' || currentUser.role === 'room_admin')) {
        ...
        // Regular users only see approved bookings
        if (bookingStatus !== 'approved') {
          return false;
        }
      }
      ...
      // For old format hourly bookings, check time slots
      if (booking.bookingType === 'hourly' && booking.time) {
        const bookingStartIndex = getTimeSlotIndex(booking.time);
  ...
  const confirmBooking = async (bookingData) => {
    ...
    const apiBookingData = {
      ...
      start_date: bookingData.startDate,
      end_date: bookingData.endDate || bookingData.startDate,
      start_time: bookingData.startTime || selectedTime || '09:00',
      end_time: bookingData.endTime || '17:00',
      expected_attendees: bookingData.attendeeCount || 1
    };

    // Ensure we have valid time values
    if (!apiBookingData.start_time) {
      apiBookingData.start_time = '09:00';
    }
    if (!apiBookingData.end_time) {
      // Default to 1 hour after start time
      const startHour = parseInt(apiBookingData.start_time.split(':')[0]);
      apiBookingData.end_time = `${String(startHour + 1).padStart(2, '0')}:00`;
    }
    ...
    try {
      ...
      // Check for conflicts on the frontend first
      const conflictingBooking = bookings.find(booking => {
        if (booking.room !== selectedRoom.id && booking.roomId !== selectedRoom.id) return false;
        if (booking.start_date !== apiBookingData.start_date && booking.date !== apiBookingData.start_date) return false;

        // Check time overlap
        const bookingStartTime = booking.start_time || booking.time;
        const bookingEndTime = booking.end_time || (booking.time ? `${parseInt(booking.time.split(':')[0]) + (booking.duration || 1)}:00` : null);

        if (bookingStartTime && bookingEndTime) {
          return (apiBookingData.start_time < bookingEndTime && apiBookingData.end_time > bookingStartTime);
        }
        return false;
      });
      ...
      newBooking = await apiCreateBooking(apiBookingData);
      ...
      if (error.message.includes('Time slot is already booked') || error.message.includes('conflicts')) {
    ...
    // Skip email notifications for now to avoid errors
    console.log('Booking created and data refreshed successfully');
